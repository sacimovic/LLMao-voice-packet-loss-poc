...
pub fn degrade_audio(samples: &[f32], packet_loss_percent: f32) -> Vec<f32> {
    degrade_audio_random(samples, packet_loss_percent)
}
...
pub fn degrade_audio_window(samples: &[f32], window_start_ms: u32, window_length_ms: u32) -> Vec<f32> {
    let mut output = samples.to_vec();
    ...
    println!("[Degradation] Zeroing window: {}ms to {}ms ({} to {} samples)", 
             window_start_ms, window_start_ms + window_length_ms, start_sample, end_sample);
